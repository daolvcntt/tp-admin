  ...
  <div class="main-content">
    <div class="row">
      <button class="btn-back" @click="returnRoomList()">
        <i style="padding-right: 5px;" class="icon-fa icon-fa-undo"/>
        Quay lại danh sách phòng
      </button>
    </div>
    <div class="row">
      <div class="col-sm-7" v-if="roomDetail" style="padding-right: 243px; text-align: center;">
      ...
      <div class="col-sm-7" v-for="(category, index) in guideBookList" :key="index">
        <div v-if="category.lang=='vi'" class="form-group" style="padding-bottom: 20px;">
          <div class="row">
            <div class="col-sm-1">
              <div class="bg-icon">
          ...
          <div class="col-sm-7" style="margin-left: 60px;">
            <label :style="errors.has('address' + index) ? 'color:red;' : ''">
            ...
            <textarea
              ...
              class="guide-descript"
              placeholder="Mô tả"
            ></textarea>
            <button
              v-if="addressMap[index] && isUpdate == 0"
              @click="createPlace(category.id, index)"
              class="btn btn-info"
            >Thêm</button>
            <button
              v-if="addressMap[index] && isUpdate == 1"
          ...
          <div class="form-group row wrapper-room panel-collapse custom-panel">
            <div class="col-sm-8" style="padding-left: 90px;padding-top: 10px;padding-right: 30px;">
              <draggable :options="{group:'people'}" class="container-form">
                <div class="list-form">
                  <p
                    v-if="place.guidebook_category_id == (index+1)"
  ...
  methods: {
    ...
    showInfoWindow(place) {
      if (this.isUpdate == 1) {
        ...
        this.infoWindowPos.lat = parseFloat(
          this.currentPlace.geometry.location.lat(),
        );
        this.infoWindowPos.lng = parseFloat(
    ...
    showInfoPlace(place) {
      ...
      this.addressMap[place.guidebook_category_id - 1] = this.currentPlace.name;
      ...
      this.description[place.guidebook_category_id - 1] = place.description;
      (this.infoWindowPos = {
        lat: parseFloat(place.latitude),
        lng: parseFloat(place.longitude),
      }),
      ...
      this.placeRecommendation.guidebook_category_id =
        place.guidebook_category_id;
    },
    clearForm(index) {
