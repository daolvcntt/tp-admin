          ...
          <div class="row">
            <div class="col-md-6">
              <div class="form-group row">
                <label for="firstName" class="col-sm-4 col-form-label">Tên chương trình khuyến mãi</label>
                <div class="col-sm-8">
                  <input
                    v-model="name"
                    id="firstName"
        ...
        <table class="table table-bordered">
          ...
          <tbody>
            <tr v-for="(coupon, index) in listCoupon" :key="index">
              ...
              <td>
                <p v-for="d in coupon.settings.days" :key="d">{{d}}</p>
              </td>
              <td>
                <div class="btn-group" role="group" aria-label="First group">
                  <button
                    @click="openUpdateCoupon(coupon.id)"
                    ...
                    v-tooltip.top-center="'Cập nhật discount'"
                    type="button"
          ...
          <tbody>
            ...
            >
              ...
              <td class="cell-content">
                <div class="content">
                  <div class="content-name">{{ promotion.name }}</div>
                </div>
              </td>
              <td width="20%">
              ...
              <td width="10%">
                <div>
                  <button
                    v-if="checkDueDate(promotion.date_end,promotion)"
                    disabled="true"
                    type="button"
                    class="btn btn-sm btn-icon btn-secondary pl-3 pr-4"
                  >Hết hạn</button>
                  <button
                    v-else-if="promotion.status == 1"
                    type="button"
                  ...
                  <button
                    v-else
                    type="button"
                    @click="updateStatus(0,promotion)"
                    v-tooltip.top-center="'Click để thay đổi trạng thái'"
              ...
              <td width="10%">
                <div class="btn-group" role="group" aria-label="First group">
                  <button
                    @click="openListCoupon(promotion)"
                    type="button"
                  ...
                  <button
                    @click="openUpdatePromotion(promotion.id)"
                    type="button"
                  ...
                  <button
                    @click="openDetailPromotion(promotion.id)"
                    type="button"
  ...
  methods: {
    ...
    async getPromotions({ page, filter, sort }) {
      try {
        ...
        if (error) {
          window.toastr["error"](
            "Dữ liệu danh sách chương trình khuyến mãi hiện thời chưa có sẵn, vui lòng thử lại sau",
            "Error"
          );
        }
      }
    ...
    async getPromotionStatus() {
      try {
        ...
        if (error) {
          window.toastr["error"](
            "Dữ liệu danh sách trạng thái khuyến mãi hiện thời chưa có sẵn, vui lòng thử lại sau",
            "Error"
          );
        }
      }
