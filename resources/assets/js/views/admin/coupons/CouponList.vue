              ...
              <td class="cell-content" width="10%">
                <div class="content">
                  <button
                    ...
                    type="button"
                    @click="updateCouponMinor('status',coupon)"
  ...
  methods: {
    ...
    async getCoupons({ page, filter, sort }) {
      try {
        ...
        if (error) {
          console.log(error)
          window.toastr["error"]("Không lấy được dữ liệu mã giảm giá", "Error");
        }
      }
    },
    async getPromotionStatus() {
      try {
        ...
        if (error) {
          window.toastr["error"]("Không lấy được dữ liệu trạng thái khuyến mãi", "Error");
        }
      }
    },
    ...
    async updateCouponMinor(option, coupon) {
      if (option === "status") {
        let response = await axios
          .put(`coupons/single-update/${coupon.id}?option=${option}`, {
