<template>
  <div class="main-content">
    <div class="page-header">
      <h3 class="page-title">Tỉnh/thành phố</h3>
      <ol class="breadcrumb">
          ...
          <div class="row">
            <div class="col-md-12">
              <div class="form-group row">
                <label for="firstName" class="col-sm-2 col-form-label">Tên tỉnh/thành phố</label>
                <div class="col-sm-10">
                  <input
                    v-model="name"
                    id="firstName"
            ...
            <tr v-for="(city,index) in cities" :key="index">
              ...
              <td class="cell-fav">
                <div class="content-subject mb-3">
                  <button
                    @click="updateCityMinor('status',city)"
                    v-if="city.status == 0"
                    type="button"
                    v-tooltip.top-center="'Tỉnh/thành phố không khả dụng (click để cập nhật)'"
                  ...
                  <button
                    ...
                    v-if="city.status == 1"
                    type="button"
                    v-tooltip.top-center="'Tỉnh/thành phố khả dụng (click để cập nhật)'"
                ...
                <div class="content-subject mb-3">
                  <button
                    @click="updateCityMinor('hot',city)"
                    v-if="city.hot == 0"
                    type="button"
                    v-tooltip.top-center="'Tỉnh/thành phố không nổi bật (click để cập nhật)'"
                    class="btn btn-xs btn-icon btn-danger mailbox-action"
                  >
                    <i class="icon-fa icon-fa-times"></i>
                  </button>
                  <button
                    ...
                    v-if="city.hot == 1"
                    type="button"
                    v-tooltip.top-center="'Tỉnh/thành phố nổi bật (click để cập nhật)'"
                    class="btn btn-xs btn-icon btn-success mailbox-action"
                  >
                    <i class="icon-fa icon-fa-check"></i>
                  </button>
              ...
              <td width="20%" style="text-align: center;">
                <div class="btn-group" role="group" aria-label="First group">
                  <button
                    @click="openUpdateCity(city.id)"
                    type="button"
                    v-tooltip.top-center="'Cập nhật tỉnh/thành phố'"
  ...
  methods: {
    async getCities({ page }) {
      try {
        ...
        if (error) {
          window.toastr['error'](
            'Dữ liệu tỉnh/thành phố hiện thời chưa có sẵn, vui lòng thử lại sau',
            'Error',
          );
        }
      }
    },
    async updateCityMinor(option, city) {
      if (option === 'status') {
        let response = await axios
          .put(`cities/prop-update/${city.id}?option=${option}`, {
        ...
        if (city.priority == 0) {
          priorityStatus = 1;
        } else if (city.priority == 1) {
          priorityStatus = 2;
        } else if (city.priority == 2) {
          priorityStatus = 3;
        } else if (city.priority == 3) {
          priorityStatus = 0;
        }
        let response = await axios
          .put(`cities/prop-update/${city.id}?option=${option}`, {
